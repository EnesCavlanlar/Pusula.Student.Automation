...
namespace Pusula.Student.Automation.Attendances
{
        ...
        IAttendanceAppService
    {
        ...
        {
            // Attendance -> Enrollment -> (CourseId, StudentId)
        ...
        {
            var entity = await Repository.GetAsync(id);
            await EnsureAttendanceAccessibleAsync(entity);                // mevcut kayda erişim
            await EnsureEnrollmentAccessibleForWriteAsync(input.EnrollmentId); // hedefe yetki
